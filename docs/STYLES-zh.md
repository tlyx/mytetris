...
本文档描述 TaoClear 代码库使用的编程风格与约定。本文是 [STYLES.md](STYLES.md) 的中文翻译;代码注释与文档字符串使用中文(项目语言),提交信息与英文版指南使用英文。修改代码时请遵循以下规则以保持风格一致。
...
- 旋转使用**官方 SRS 踢位表**,以 `(from_rotation, to_rotation)` 为键(每过渡 5 个候选,`(0,0)` 恒为第一候选 = 原地旋转),坐标翻转为底部原点。目前只实现顺时针 4 个过渡;逆时针待未来支持。
- **T-spin**(指南标准):判定需要同时满足 T 块**3 角规则**——中心四对角 ≥3 格被占(`is_t_spin`,与旋转无关,中心恒为形状原点)——以及 `last_was_rotation`:旋转必须是字面上的最后动作,任何成功的移动/软降都会清除标志。**硬降只有真的移动了方块(距离 > 0)才清除**——贴地硬降(距离 0)只是"立即锁定"的确认键,旋转依然成立。因此**空中旋转后硬降砸进槽位不算 T-spin,贴地入槽后以 0 距离硬降确认则算**。计分走 `T_SPIN_TABLE`(无消行 100,单 800,双 1200,三 1600,×level);连击加成正交叠加。
