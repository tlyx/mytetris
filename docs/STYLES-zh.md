...
- **T-spin**(指南标准):判定需要同时满足 T 块**3 角规则**——中心四对角 ≥3 格被占(`is_t_spin`,与旋转无关,中心恒为形状原点)——以及 `last_was_rotation`:旋转必须是字面上的最后动作,任何成功的移动/软降/硬降都会清除标志。**旋转后硬降刻意不算 T-spin**。计分走 `T_SPIN_TABLE`(无消行 100,单 800,双 1200,三 1600,×level);连击加成正交叠加。
